<template>
	<div>
		<Modal id="one" :fullscreen="fullscreen" v-model="isThreeViewBool" footer-hide :draggable="draggable" :closable="false" >
...
<script>
	export default {
		...
		computed: {
			...
			isThreeViewBool: {
				get() {
					return this.$store.state.control.threeViewBool;
				},
				set(v) {
					this.$store.commit("setFalseThreeVB");
				}
			}
		},
		methods: {
