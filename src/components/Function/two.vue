<template>
	<div>
		<Modal id="one" :fullscreen="fullscreen" v-model="isTwoViewBool" footer-hide :draggable="draggable" :closable="false" >
...
<script>
	export default {
		...
		computed: {
			...
			isTwoViewBool: {
				get() {
					return this.$store.state.control.twoViewBool;
				},
				set(v) {
					this.$store.commit("setFalseTwoVB");
				}
			},
		},
		methods: {
